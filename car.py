...
def write_sql(shopid):
    try:
        ...
        df.to_sql(con = engine_poi, name = "car_history", if_exists = 'append', flavor = "mysql",index=False)
    except:
...
#
# while True:
#     gotten_poi = pd.read_sql("car_latest",engine_poi)
#     dl = pd.read_sql("proxys", engine_proxy)
#     total_ids = set(pd.read_sql("car_shopids", engine_poi)["shopid"])
#     gotten_ids = set(pd.read_sql("car_latest",engine_poi)["shopid"])
...
#         engine_poi.execute("SET SQL_SAFE_UPDATES = 0;")
#         clean_tb("car_history")
#         get_latest("car")
#         if n2>n1:
#             engine_poi.execute("INSERT update_logs VALUES(\"car\",%d,\"%s\");"%(n2-n1,time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time()))))
#         print("已经完成一轮更新！")
#     else:
#         print("暂时没有需要更新的！")
#     print("请等待下一轮更新！")
...
while True:
    dl = pd.read_sql("proxys", engine_proxy)
    total_ids = set(pd.read_sql("car_shopids", engine_poi)["shopid"])
    gotten_ids = set(pd.read_sql("car_history", engine_poi)["shopid"])
    remained_id = total_ids - gotten_ids
    remained_id = list(remained_id)
    if len(remained_id)>0:
        insert_food(remained_id)
